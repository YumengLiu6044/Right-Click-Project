      ...
      <Dialog open={showSuccess} onOpenChange={setShowSuccess}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-xl">Work Order Created!</DialogTitle>
            <DialogDescription className="text-center">
              Your work order has been successfully created and saved.
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col items-center gap-4 pt-4">
            <div className="flex items-center gap-2 p-4 bg-secondary rounded-lg w-full justify-center">
              <FileText className="h-5 w-5 text-primary" />
              <span className="font-mono font-bold text-lg">{workOrderId}</span>
            </div>
            <p className="text-sm text-muted-foreground text-center">
              {customer?.name} • {equipment.length} item(s) • {labor.reduce((sum, l) => sum + l.hours, 0)}h labor
            </p>
            <div className="flex gap-2 w-full">
              <Button variant="outline" className="flex-1" onClick={() => setShowSuccess(false)}>
                View Details
              </Button>
              <Button className="flex-1" onClick={handleNewWorkOrder}>
                New Work Order
              </Button>
            </div>
          </div>
